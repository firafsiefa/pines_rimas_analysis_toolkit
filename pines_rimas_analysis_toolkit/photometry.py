...
from photutils.psf import PSFPhotometry, extract_stars, IterativePSFPhotometry,  EPSFBuilder, EPSFFitter #IntegratedGaussianPRF, 
...
def fixed_aper_phot(short_name, ap_radii, filter, an_in=12., an_out=30., gain=1.8, qe=0.9, force_output_path='', interpolate_nans=False):
    ...
    centroided_sources = centroided_sources1[~np.isnan(pd.to_numeric(centroided_sources1[short_name+' Image X'], errors='coerce'))].reset_index(drop=True)
    ...
    reduced_files = reduced_files1[~np.isnan(pd.to_numeric(centroided_sources1[short_name+' Image X'], errors='coerce'))]
...
def manual_ref_star_chooser(short_name, profile_data, positions=[], restore=False, source_detect_plot=False, force_output_path=''):
    """Chooses suitable reference stars for a target in a specified source_detect_image.
    ...
    #sources = detect_sources(source_detect_image_path, source_detect_seeing, positions,
     #                        edge_tolerance, plot=source_detect_plot, thresh=3.0)
    sources = pd.DataFrame({'x_center':positions[:,0], 'y_center': positions[:,1]})
