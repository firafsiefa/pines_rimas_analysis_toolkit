...
def bg_2d(data, box_size=50):
    ...
    bkg = Background2D(data, (box_size, box_size), filter_size=(
...
def bpm_chooser(bpm_path, filter, header):
    ...
    bpm_ind = np.where(np.array(bpm_date_distances) ==
                       min(np.array(bpm_date_distances)))[0][0]
...
def dark(date, exptime, band, dark_start=0, dark_stop=0, upload=False, delete_raw=False, sftp='', force_output_path=''):
    ...
    if type(sftp) == pysftp.Connection:
        ...
        if data_path == '':
            print(
                'ERROR: specified date not found in any run on pines.bu.edu:data/raw/mimir/\n')
            return
        else:
            ...
            if (dark_stop != 0):
                ...
                # Identify dark files.
                dark_inds = np.where((log['Target'] == 'Dark') & (
                    log['Filename'] != 'test.fits') & (log['Exptime'] == exptime))[0]
                # Set guarantees we only grab the unique files that have been identified as flats, in case the log bugged out.
                dark_files = natsorted(list(set(log['Filename'][dark_inds])))
            ...
            for j in range(len(dark_files)):
                if not (dark_path/dark_files[j]).exists():
                    sftp.get(dark_files[j], os.path.join(
                        pines_path, dark_path/dark_files[j]))
                    print('Downloading {} to {}, {} of {}.'.format(
                        dark_files[j], dark_path, j+1, len(dark_files)))
                else:
                    print('{} already in {}, skipping download.'.format(
                        dark_files[j], dark_path))
            print('')
                ...
                for yy in range(int(box_l/2), shape[1]-int(box_l/2)):
                    ...
                    if (not np.isnan(master_flat[yy, xx])):
                        ...
                        if master_flat[yy, xx] < (np.mean(neighbor_vals) - clip_lvl*np.std(neighbor_vals)):
                            ...
                            master_flat[yy, xx] = np.nan
                            num_flagged += 1
            iteration += 1
            total_flagged += num_flagged
            print('Iteration {}: {} new dead pixels identified, {} dead pixels total.'.format(
                iteration, num_flagged, total_flagged))
            print('')
...
def dome_flat_field(date, band, lights_on_start=0, lights_on_stop=0, lights_off_start=0, lights_off_stop=0, upload=False, delete_raw=False, sftp='', force_output_path=''):
        ...
        if data_path == '':
            ...
            for j in range(len(flat_files)):
                if not (dome_flat_raw_path/flat_files[j]).exists():
                    ...
                    print('Downloading {} to {}, {} of {}.'.format(
                        flat_files[j], dome_flat_raw_path, j+1, len(flat_files)))
                else:
                    print('{} already in {}, skipping download.'.format(
    ...
    if len(bad_locs > 0):
        print('Found {} bad lights-on flats: {}.\nExcluding from combination step.\n'.format(
            len(bad_locs), np.array(lights_on_files)[bad_locs]))
        time.sleep(2)
    ...
    master_flat_error = np.sqrt(
        master_flat_lights_on_stddev**2 + master_flat_lights_off_stddev**2)
    master_flat_error = master_flat_error / master_flat_norm

    # Ourput flat files.
    output_filename = pines_path / \
        ('Calibrations/Flats/Domeflats/'+band +
    ...
    print('dome_flat_field runtime: ', np.round(
        (time.time()-t1)/60, 1), ' minutes.')
    print('Done!')
...
def get_calibrations(sftp, pines_path):
    ...
    calibration_directories = ['Bad Pixel Masks', 'Darks', 'Dead Pixel Masks',
                               'Flats', 'Hot Pixel Masks', 'Kokopelli Mask', 'Variable Pixel Masks']
    for i in range(len(calibration_directories)):
        ...
        if calibration_directories[i] != 'Flats':
            ...
            if calibration_directories[i] == 'Darks':
                local_path = pines_path / \
                    ('Calibrations/' +
                     calibration_directories[i]+'/Master Darks/')
            else:
                local_path = pines_path / \
                    ('Calibrations/'+calibration_directories[i]+'/')
            ...
            for j in range(len(bands)):
                ...
                files = sftp.listdir()
                local_path = pines_path / \
                    ('Calibrations/Flats/Domeflats/'+bands[j]+'/Master Flats/')
...
def get_raw_science_files(sftp, target_name, force_output_path=''):
    ...
    dates = [int(i) for i in list(
        set([str.split(file_names[i], '.')[0] for i in range(len(file_names))]))]
    dates = np.array(dates)[np.argsort(dates)]
    comp_dates = np.array([int(file_names[i].split('.')[0])
                          for i in range(len(file_names))])
    print('Found ', len(file_names), ' raw files for ',
          target_name, ' on ', len(dates), ' dates.')
        ...
        for j in range(len(night_dirs)):
            ...
            if night_check in dates:
                ...
                if not log_name in sftp.listdir('/data/logs/'):
                    ...
                    sftp.put(pines_path/('Logs/'+log_name),
                             '/data/logs/'+log_name)

                date_holder_ind = np.where(
                    np.array(dates) == night_check)[0][0]
                files = date_holder[date_holder_ind]
                print('Downloading {} data for {} to {}!'.format(
                    night_check, target_name, raw_data_path))
                pbar = ProgressBar()
                for k in pbar(range(len(files))):
                    ...
                    #Move the file into a filter subdirectory.
                    filter = fits.open(raw_data_path/files[k])[0].header['FILTNME2']
                    if not os.path.exists(raw_data_path/filter):
                        os.mkdir(raw_data_path/filter)
                    os.replace(raw_data_path/files[k], raw_data_path/(filter+'/'+files[k]))
                sftp.chdir('..')
        sftp.chdir('..')
    ...
    for i in range(len(dates)):
        ...
        print('Downloading {} to {}.'.format(
            log_name, pines_path/('Logs/'+log_name)))
        sftp.get(log_name, pines_path/('Logs/'+log_name))
                ...
                for k in range(len(files)):
                    ...
                    if not (reduced_data_path/download_filename).exists():
                        if download_filename in files_in_path:
                            print('Downloading to {}, {} of {}'.format(
                                reduced_data_path/download_filename, file_num, len(file_names)))
                            sftp.get(download_filename,
                                     reduced_data_path/download_filename)
                        else:
                            print('A reduced image does not yet exist for {}, ask an administrator to make one!'.format(
                                files[k]))
                    else:
                    ...
                    #Move the file into a filter subdirectory.
                    filter = fits.open(reduced_data_path/download_filename)[0].header['FILTNME2']
                    if not os.path.exists(reduced_data_path/filter):
                        os.mkdir(reduced_data_path/filter)
                    os.replace(reduced_data_path/download_filename, reduced_data_path/(filter+'/'+download_filename))
...
def hot_pixels(date, exptime, band, saturation=45000., upload=False, sftp='', force_output_path=''):
    ...
    if dark_file == '':
        raise RuntimeError('No dark stddev files found on disk with date ' +
                           date+' and exposure time '+str(exptime)+' seconds!')
    ...
    for i in range(len(box_ls)):
        ...
        while (num_flagged > 1):
            ...
            iteration += 1
            total_flagged += num_flagged
            print('Iteration {}: {} new hot pixels identified, {} hot pixels total.'.format(
                iteration, num_flagged, total_flagged))
            print('')
...
def make_calibrations(sftp, exptimes, bands, dark_dates, flat_dates, dark_starts=[], dark_stops=[], lights_on_starts=[],
    ...
    assert len(bands) == len(
        flat_dates), 'Num. of elements in the bands list does not equal the num. of elements of the flat_dates list!\n You must have one flat_dates entry for every bands entry.'
    assert len(exptimes) == len(
        dark_dates), 'Num. of elements in the exptimes list does not equal the num. of elements of the dark_dates list!\n You must have one dark_dates entry for every exptimes entry.'
    ...
    if dark:
        for i in range(len(exptimes)):
            ...
            dark_path = calibration_path / \
                ('Darks/Master Darks/master_dark_' +
                 str(exptime)+'_s_'+dark_date+'.fits')
            # If the master dark for this exptime/date already exist, skip!
            if dark_path.exists():
                print('{} already exists in {}, skipping.'.format(
                    dark_path.name, calibration_path))
            else:
                print(
                    'Making master dark for {}-s exposure time on {}.'.format(exptime, dark_date))
    ...
    if flat:
        for i in range(len(bands)):
            ...
            band = bands[i]
            flat_path = calibration_path / \
                ('Flats/Domeflats/'+band+'/Master Flats/master_flat_' +
                 band+'_'+flat_date+'.fits')
            if flat_path.exists():
                print('{} already exists in {}, skipping.'.format(
                    flat_path.name, calibration_path))
            else:
    ...
    if variable_pixels:
        for i in range(len(exptimes)):
            dark_date = dark_dates[i]
            exptime = float(exptimes[i])
            vpm_path = calibration_path / \
                ('Variable Pixel Masks/vpm_'+str(exptime)+'_s_'+dark_date+'.fits')
            if vpm_path.exists():
                print('{} already exists in {}, skipping.'.format(
                    vpm_path.name, calibration_path))
            else:
                print(
    ...
    if hot_pixels:
        for i in range(len(exptimes)):
            dark_date = dark_dates[i]
            exptime = float(exptimes[i])
            hpm_path = calibration_path / \
                ('Hot Pixel Masks/hpm_'+str(exptime)+'_s_'+dark_date+'.fits')
            if hpm_path.exists():
                print('{} already exists in {}, skipping.'.format(
                    hpm_path.name, calibration_path))
            else:
                print(
    ...
    if dead_pixels:
        for i in range(len(bands)):
            flat_date = flat_dates[i]
            band = bands[i]
            dpm_path = calibration_path / \
                ('Dead Pixel Masks/dpm_'+band+'_'+flat_date+'.fits')
            if dpm_path.exists():
                print('{} already exists in {}, skipping.'.format(
                    dpm_path.name, calibration_path))
            else:
                print(
    ...
    if bpm:
        for i in range(len(exptimes)):
            ...
            for j in range(len(bands)):
                ...
                band = bands[j]
                bpm_path = calibration_path / \
                    ('Bad Pixel Masks/bpm_'+band+'_' +
                     str(exptime)+'_s_'+flat_date+'.fits')
                if bpm_path.exists():
                    print('{} already exists in {}, skipping.'.format(
                        bpm_path.name, calibration_path))
                else:
                    print(
...
def master_dark_chooser(dark_path, band, header):
    ...
    if (len(possible_darks) == 0):
        ...
        dark_ind = np.where(np.array(dark_date_distances) ==
                            min(np.array(dark_date_distances)))[0][0]
...
def master_dark_stddev_chooser(dark_std_path, band, header):
    ...
    if (len(possible_dark_stds)) == 0:
        ...
        dark_std_ind = np.where(np.array(dark_std_date_distances) == min(
            np.array(dark_std_date_distances)))[0][0]
...
def master_flat_chooser(flats_path, filter, header):
    ...
    if (len(possible_flats)) == 0:
        ...
        flat_ind = np.where(np.array(flat_date_distances) ==
...
def reduce(short_name, band, filter='', delete_raw=False, manual_flat_path='', manual_dark_path='', manual_bpm_path='', linearity_correction=False, force_output_path=''):
    ...
    :type short_name: str
    :param filter: the filter you want to reduce data for. Use '' to reduce all data. 
    :type filter: str
    ...
    dark_path = pines_path/('Calibrations/Darks/')
    reduced_top_level_path = pines_path/('Objects/'+short_name+'/reduced')
    ...
    for raw_sub_dir in raw_sub_dirs:
        ...
        raw_files = [Path(i) for i in natsorted(glob.glob(os.path.join(raw_sub_dir, '*.fits')))]

        # Now begin the loop to load and reduce the raw science data
        print("Reducing {}-band data for {}.".format(raw_sub_dir.name, short_name))
        ...
        for i in pbar(range(np.size(raw_files))):
            ...
            if linearity_correction:
                ...
                median_linear_count_rate = np.nanmedian(
                    10**log_line_coeffs[0, :, :])
                frame_raw[np.where(bad_pixel_mask == 1)] = np.nan
                measured_counts = frame_raw - master_dark
                corrected_counts = (measured_counts/(10**log_line_coeffs[0, :, :]))**(
                    1/log_line_coeffs[1, :, :]) * median_linear_count_rate
        ...
        if delete_raw:
            files_to_delete = glob.glob(os.path.join(raw_sub_dir/'*.fits'))
            breakpoint()
            for j in range(len(files_to_delete)):
                os.remove(files_to_delete[j])
...
def straggler_upload(sftp, targets):
    ...
    for i in range(len(targets)):
        ...
        for j in range(len(raw_files)):
            sftp.chdir('/data/reduced/mimir/')
            red_file = str(
                red_path)+'/'+raw_files[j].split('/')[-1].split('.fits')[0]+'_red.fits'
...
def upload_reduced_data(sftp, short_name):
    ...
    sftp.chdir('/data/reduced/mimir/')
    files_to_upload = np.array(
        natsorted(np.array([x for x in reduced_path.glob('*red.fits')])))
...
def variable_pixels(date, exptime, band, clip_lvl=5., upload=False, sftp='', force_output_path=''):
    ...
    variable_inds = np.where(master_dark_stddev >=
                             clip_lvl*np.nanmean(master_dark_stddev))
